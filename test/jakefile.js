...
const JF = require('../lib/jake/jakefile.js');

describe('jakefile', function(done)
{
	describe('findfile', function()
	{
		...
		{
			chai.expect((JF.directoryHasFile(path.resolve(__dirname, "data/sub1"), JF.defaultFilenames() ))).equal('jakefile.js');
			done();
		});
	});	
	describe('findfilerecursive', function()
	{
		it('find recursive', function(done)
		{
			...
			let fp = path.resolve(__dirname, 'data/sub1/jakefile.js');
